...
GPU = 'cuda:0'
CONF = 0.25
HEATMAP_MAX_ALPHA = 0.35
FRAMES_DIR = Path('C:/aya-expo-tools/tmp/tl6_frames')
OUT = Path('C:/aya-expo-tools/tmp/timelapse-stories-v6.mp4')
...
print('=== Timelapse Stories v6 - heatmap sutil ===')
...
print(f'Heatmap max alpha: {HEATMAP_MAX_ALPHA}')
...
for cam, _label in camera_defs:
    ...
    for i, fpath in enumerate(frames_by_cam[cam][:n]):
        ...
        if hm_max > 0.5:
            ...
            alpha = np.clip(hm_norm * 1.0, 0, HEATMAP_MAX_ALPHA)
...
for i in range(n):
    ...
    txt_center(draw, '20.03.2026  ·  14h45 — 17h39', fy+90, f_foot2, GRY)
    if logo_szt:
