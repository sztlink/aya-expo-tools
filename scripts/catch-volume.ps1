...
# Rapid fire: set to 80 then poll every 200ms
[AudioCtrl]::Set(80)
...
for ($i = 0; $i -lt 30; $i++) {
    ...
    if ($v -ne $lastVol) {
        Write-Output "$(Get-Date -Format 'HH:mm:ss.fff') CHANGED: $lastVol -> $v (after $($i*200)ms)"
        $lastVol = $v
    }
}
Write-Output "$(Get-Date -Format 'HH:mm:ss.fff') FINAL: $([int][Math]::Round([AudioCtrl]::Get()))%"
